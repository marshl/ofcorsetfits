...
describe('cupped-rib silhouette-specific scoring', () => {
  ...
  it('penalizes underbust tightness symmetrically for cupped-rib silhouettes (tight ≈ as bad as loose)', () => {
    // Cupped-rib means "both directions of underbust misfit are equally bad."
    // Under the current defaults looseness is the harsher slope, so cupped-rib
    // upgrades TIGHT (the softer default) to match it — that's where the delta
    // vs. a non-cupped baseline lives.
    //
    // Wide-rib body → negative diff (corset < body → tight) at underbust for
    // most corsets. Compare the corset's rib penalty when its silhouette is
    // 'cupped-rib' vs the same corset relabelled 'hourglass'.
    const config = defaultScoringConfig(catalog);
    const wideRibBody: Body = {
      natural_waist_in: 28,
      underbust: { circumference_in: 44, position_in: -5 },
      upper_hip: { circumference_in: 34, position_in: 4 },
      iliac: { circumference_in: 38, position_in: 7 },
    };
    ...
    if (!cuppedRib) return;
    const asCupped = rank(wideRibBody, catalog, config);
    ...
    // This test only exercises the tight case (negative diff at rib).
    ...
    const asHourglass = rank(wideRibBody, modifiedCatalog, config);
    ...
    // than the hourglass version, because tight at the rib is symmetrized up
    // to max(tight, loose) = looseness_slope.
    expect(cuppedUnderbust.penalty).toBeGreaterThan(hourglassUnderbust.penalty);
    // Sanity: ratio should be roughly looseness_slope / tightness_slope = 1.5.
